...
void __fastcall TStorageLossOrder::FormCreate(TObject *Sender)
{
	...
	StorageLossStringGrid->ColWidths[2] = 150;
...
void __fastcall TStorageLossOrder::SLSellerNameChange(TObject *Sender)
{
	...
	Qry->Close();
	Qry->SQL->Clear();
	AnsiString sql = "select id from sellers where (delete_flag = 0 or delete_flag is null) and name = '" + SLSellerName->Text.Trim() + "' and supplier_id = " + AnsiString(MainForm->ParentId);
          ...
          int ACol, int ARow, bool &CanSelect)
{
	if (ACol == 2) {
		...
		if(StorageLossStringGrid->Cells[1][ARow].Trim() != "") {
			...
			str += " where 1 = 1 and products.is_valid = 1 ";
...
//		AnsiString purchase_order_id = Cmd->InsertId;
		...
		for(unsigned i=1;i<=StorageLossStringGrid->RowCount;i++){
			...
			sql = "SELECT id, general_product_id FROM products WHERE is_valid = 1 and chinese_name = '"+ StorageLossStringGrid->Cells[2][i] +"' and supplier_id = " + AnsiString(MainForm->ParentId);
			Qry->Close();
			Qry->SQL->Clear();
			Qry->SQL->Add(sql);
			Qry->Open();
