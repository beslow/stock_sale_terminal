...
//	}
	...
	try
	{
		...
		for(unsigned i=1;i<=StringGrid1->RowCount;i++){
			...
			sql = "SELECT id, general_product_id FROM products WHERE is_valid = 1 and chinese_name = '"+ StringGrid1->Cells[2][i] +"' and supplier_id = " + AnsiString(MainForm->ParentId);
			Qry->Close();
			Qry->SQL->Clear();
			Qry->SQL->Add(sql);
			Qry->Open();
...
//					}
			...
			if(current_purchase_price==AnsiString("0")||current_purchase_price==AnsiString("0.0"))
			{

			}
			else
			{
				sql = " update stocks set last_purchase_price = " + AnsiString(current_purchase_price);
				sql += " where id = " + AnsiString(stock_id);
				Cmd->SQL->Clear();
				Cmd->SQL->Add(sql);
				Cmd->Execute();
