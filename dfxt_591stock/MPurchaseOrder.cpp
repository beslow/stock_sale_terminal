...
//	}
	...
	try
	{
		...
		for(unsigned i=1;i<=StringGrid1->RowCount;i++){
			...
			AnsiString current_ture_spec = StringGrid1->Cells[5][i].Trim();
			...
			//增加库存进出明细
			sql = " INSERT INTO `order_details` (`supplier_id`, `related_company_id`, `order_id`, `detail_type`, `detail_date`, `item_id`, `product_id`, `price`, `plan_weight`, `real_weight`, `money`, `delete_flag`, `created_at`, `updated_at`) ";
			sql += " VALUES ";
			sql += " (";
			sql += AnsiString(MainForm->ParentId) + ", ";
			sql += AnsiString(SellerId) + ", ";
